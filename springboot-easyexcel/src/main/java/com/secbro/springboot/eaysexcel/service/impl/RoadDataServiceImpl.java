...
import com.secbro.springboot.eaysexcel.entity.RoadData;
...
public class RoadDataServiceImpl implements RoadDataService {
    ...
    //  多个sheet导入
    @Override
    public void uploadManySheet(MultipartFile file,HttpServletRequest request,HttpServletResponse response) {
        Message message = new Message();
        EasyExcelUtils easyExcelUtils = new EasyExcelUtils(RoadData.class);  //创建工具类时传递class，用于后面比对表头使用
        try {
